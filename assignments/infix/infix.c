...
void createoptr(struct optrstack *s);
void createopnd(struct opndstack *s);

void createoptr(struct optrstack *s){
	s = (struct optrstack *)malloc(sizeof(optrstack));
	s->a = (char *)malloc(sizeof(char) * MAX);
	s->i = 0;
}
void createopnd(struct opndstack *s){
	s = (struct opndstack *)malloc(sizeof(opndstack));
...
printf("%d\n", i++);
        ...
        // Closing brace encountered, solve entire brace
            ...
            // to top two elements in values stack
				while (!emptyOptr(&optr) && (precedence(t->op) <= precedence(Top(&optr)))){
					a = PopOpnd(&value);
					b = PopOpnd(&value);
					op = PopOptr(&optr);
					if(op == INT_MIN || a == INT_MIN || b == INT_MIN){
